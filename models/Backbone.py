...
class I3D_backbone(nn.Module):
    ...
    def forward(self, video_1, video_2):
        ...
        # print("total_feature_re shape:", total_feature.shape)
        total_feamap = total_feamap.reshape(
            len(start_idx), len(total_video), C, T, H, W
        ).transpose(
            0, 1
        )  # [4, 9, 1024, 2, 4, 4]
        # print("total_feamap shape:", total_feamap.shape)

        com_feature_12 = torch.cat(
            (
                total_feature[: total_feature.shape[0] // 2],
                total_feature[total_feature.shape[0] // 2 :],
            ),
            2,
        )  # [2, 9, 2048]
        # print("com_feature_12 shape:", com_feature_12.shape)
        com_feamap_12 = torch.cat(
            (
                total_feamap[: total_feamap.shape[0] // 2],
                total_feamap[total_feamap.shape[0] // 2 :],
            ),
            2,
        )  # [2, 9, 2048, 2, 4, 4]
        # print("com_feamap_12 shape:", com_feamap_12.shape)
