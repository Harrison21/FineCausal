...
class FineDiving_Pair_Dataset(torch.utils.data.Dataset):
    def __init__(self, args, subset, transform):
        ...
        if self.action_number_choosing:
            self.ac_preprocess()
            self.ac_check_exemplar_dict()
        # if self.DD_choosing:
        #     self.dd_preprocess()
        #     self.dd_check_exemplar_dict()

    def ac_preprocess(self):
    ...
    def dd_preprocess(self):
        for item in self.train_dataset_list:
            difficulty = self.data_anno.get(item)[2]
    ...
    def ac_check_exemplar_dict(self):
        if self.subset == "train":
    ...
    def load_video(self, video_file_name):
        ...
        # print("image_frame_idx:", image_frame_idx)
        ...
        frames_labels = [
            self.data_anno.get(video_file_name)[4][i] for i in image_frame_idx
        ]
    ...
    def __getitem__(self, index):
        ...
        data = {}
        data["video"], data["video_mask"], data["transits"], data["frame_labels"] = (
