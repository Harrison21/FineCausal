...
class I3D_VOS(torch.nn.Module):
    def __init__(self, num_classes, modality="rgb", dropout_prob=0, name="inception"):
        ...
        # Upsampling Module
        self.side_prep1 = torch.nn.Conv3d(
            in_channels=64, out_channels=16, kernel_size=1, stride=1, bias=False
        )
        self.score_dsn1 = torch.nn.Conv3d(
            in_channels=16, out_channels=1, kernel_size=1, stride=1, padding=0
        )
        self.upsample1 = torch.nn.ConvTranspose3d(
            in_channels=16,
            out_channels=16,
            kernel_size=(4, 4, 4),
            stride=(2, 2, 2),
        ...
        self.upsample1_ = torch.nn.ConvTranspose3d(
            in_channels=1,
            out_channels=1,
            kernel_size=(4, 4, 4),
            stride=(2, 2, 2),
        ...
        self.side_prep2 = torch.nn.Conv3d(
            in_channels=192, out_channels=16, kernel_size=1, stride=1, bias=False
        )
        self.score_dsn2 = torch.nn.Conv3d(
            in_channels=16, out_channels=1, kernel_size=1, stride=1, padding=0
        )
        self.upsample2 = torch.nn.ConvTranspose3d(
            in_channels=16,
            out_channels=16,
            kernel_size=(4, 8, 8),
            stride=(2, 4, 4),
        ...
        self.upsample2_ = torch.nn.ConvTranspose3d(
            in_channels=1,
            out_channels=1,
            kernel_size=(4, 8, 8),
            stride=(2, 4, 4),
        ...
        self.side_prep3 = torch.nn.Conv3d(
            in_channels=480, out_channels=16, kernel_size=1, stride=1, bias=False
        )
        self.score_dsn3 = torch.nn.Conv3d(
            in_channels=16, out_channels=1, kernel_size=1, stride=1, padding=0
        )
        self.upsample3 = torch.nn.ConvTranspose3d(
            in_channels=16,
            out_channels=16,
            kernel_size=(4, 16, 16),
            stride=(2, 8, 8),
        ...
        self.upsample3_ = torch.nn.ConvTranspose3d(
            in_channels=1,
            out_channels=1,
            kernel_size=(4, 16, 16),
            stride=(2, 8, 8),
        ...
        self.side_prep4 = torch.nn.Conv3d(
            in_channels=832, out_channels=16, kernel_size=1, stride=1, bias=False
        )
        self.score_dsn4 = torch.nn.Conv3d(
            in_channels=16, out_channels=1, kernel_size=1, stride=1, padding=0
        )
        self.upsample4 = torch.nn.ConvTranspose3d(
            in_channels=16,
            out_channels=16,
            kernel_size=(8, 32, 32),
            stride=(4, 16, 16),
        ...
        self.upsample4_ = torch.nn.ConvTranspose3d(
            in_channels=1,
            out_channels=1,
            kernel_size=(8, 32, 32),
            stride=(4, 16, 16),
    ...
    def forward(self, inp):  # [18, 3, 16, 112, 112]
        ...
        # [36, 1024, 2, 4, 4]
        feature = self.avg_pool(out)  # [36, 1024, 1, 1, 1]
    ...
    def visualizeActivations(self, out):
        ...
        for i in range(0, number_frames):
            ...
            features_current = np.reshape(
                features_current, (number_channels, rows, cols)
            )
